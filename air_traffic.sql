...
-- Schema above has been modified below, to allow updates to be easier
CREATE TABLE tickets
(
  id SERIAL PRIMARY KEY,
  first_name TEXT NOT NULL,
...
INSERT INTO tickets
  (first_name, last_name, seat, flight_details_id)
VALUES
...
-- Sameple queries
SELECT airline_id, airlines.name AS airlines, count(*)
FROM airlines
JOIN flight_details ON airlines.id = airline_id
GROUP BY airlines.name, airline_id
ORDER BY count(*) DESC;
